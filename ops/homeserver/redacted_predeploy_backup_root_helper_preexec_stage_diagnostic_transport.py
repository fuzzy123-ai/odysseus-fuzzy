#!/usr/bin/env python3
"""Published transport for the root-helper's fixed-stage preexec diagnostic."""
...
DIAGNOSTIC_PATH = "ops/homeserver/redacted_predeploy_backup_root_helper_preexec_stage_diagnostic.py"
PUBLISHED_DIAGNOSTIC_SHA256 = "7f41d477b726d8e4f196edf1bf731c3b7b70d9ebf9be5c2ffc3b7bbb9d6c235f"
...
expected="7f41d477b726d8e4f196edf1bf731c3b7b70d9ebf9be5c2ffc3b7bbb9d6c235f"
...
name="_odysseus_pinned_root_helper_preexec_stage_diagnostic_"; module=types.ModuleType(name); module.__file__="<pinned-root-helper-preexec-stage-diagnostic>"; sys.modules[name]=module
try:
...
def _unknown() -> dict[str, Any]:
    return diagnostic.envelope("unknown", "timeout", "none", locked=True, bound=True, invoked=True)


def collect_published_root_helper_preexec_stage_diagnostic(*, execute: bool = False, runner: Callable[..., Any] = subprocess.run) -> dict[str, Any]:
    if execute is not True: return diagnostic.envelope("blocked", "execution_disabled", "none")
    source = _published_blob(runner)
    if source is None: return diagnostic.envelope("blocked", "preflight_failed", "none")
    ...
    if len(payload) > MAX_STDIN_BYTES: return diagnostic.envelope("blocked", "preflight_failed", "none")
...
def main() -> int:
    print(json.dumps(diagnostic.envelope("blocked", "execution_disabled", "none"), ensure_ascii=True, sort_keys=True, separators=(",", ":")))
    return 1
