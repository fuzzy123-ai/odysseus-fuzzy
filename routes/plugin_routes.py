...
def setup_plugin_routes() -> APIRouter:
    ...
    def _safe_plugin_script(plugin_id: str, value: object) -> str | None:
        if not isinstance(value, str):
            return None
        if not (value.startswith("/") and not value.startswith("//") and value.endswith(".js")):
            return None
        expected_prefix = f"/api/plugins/{plugin_id}/web/"
    ...
    async def ui_loader():
        """Load browser-side UI contributions for currently loaded plugins."""
        plugins = []
        for rec in sorted(_mgr().records.values(), key=lambda r: r.plugin_id):
            if not (rec.enabled and rec.status == "loaded"):
                continue
            ui = rec.public().get("ui") or {}
            script = _safe_plugin_script(rec.plugin_id, ui.get("script"))
            if not script:
                continue
            plugins.append({
                "id": rec.plugin_id,
                "name": rec.manifest.get("name", rec.plugin_id),
                "version": rec.manifest.get("version", ""),
                "script": script,
            })
        payload = json.dumps(plugins, ensure_ascii=True)
        body = f"""// Generated by Odysseus plugin system. Do not edit.
const plugins = {payload};
window.OdysseusPluginUis = plugins;
await Promise.allSettled(plugins.map(async (plugin) => {{
  try {{
    await import(plugin.script);
  }} catch (error) {{
    console.error(`[Plugin UI] Failed to load ${{plugin.id}}`, error);
  }}
}}));
"""
        return Response(
