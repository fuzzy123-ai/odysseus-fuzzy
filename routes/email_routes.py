...
from routes.email_ai_helpers import (
    ai_reply_response as _ai_reply_response,
    extract_writing_style_response as _extract_writing_style_response,
    summarize_email_response as _summarize_email_response,
)
...
def setup_email_routes():
    ...
    async def extract_writing_style(req: ExtractStyleRequest, owner: str = Depends(require_owner)):
        ...
        """
        return await _extract_writing_style_response(
            req,
            owner=owner,
            imap_factory=_imap,
            quote_folder=_q,
            detect_sent_folder=_detect_sent_folder,
    ...
    async def summarize_email(data: dict, owner: str = Depends(require_owner)):
        ...
        return await _summarize_email_response(
            data,
            owner=owner,
            db_path=SCHEDULED_DB,
            assert_owns_account=_assert_owns_account,
            imap_factory=_imap,
            quote_folder=_q,
            imap_uid_fetch=_imap_uid_fetch,
    ...
    async def ai_reply(data: dict, owner: str = Depends(require_owner)):
        ...
        return await _ai_reply_response(
            data,
            owner=owner,
            db_path=SCHEDULED_DB,
            load_settings=_load_settings,
            ...
            extract_reply=_extract_reply,
            apply_email_style_mechanics=_apply_email_style_mechanics,
            pre_retrieve_context=_pre_retrieve_context,
            fetch_sender_thread_context=_fetch_sender_thread_context,
            base_prompt=_EMAIL_REPLY_SYS_PROMPT_BASE,
        )
