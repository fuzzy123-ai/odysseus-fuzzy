...
from .derived_index import DERIVED_INDEX_PATH, build_derived_index, derived_index_status
from .memory_ledger import LEDGER_DB_PATH, memory_ledger_status, sync_memory_ledger
from .query_layer import QUERY_CACHE_PATH, answer_query, query_layer_status
...
def run_rebuild_proof(
    ...
    *,
    ...
    query_result = answer_query(vault_dir, query, top_k=top_k, path_prefix=path_prefix) if query else None
    query_status = query_layer_status(vault_dir)
    cache = dict(query_status.get("cache") or {})
    payload = {
        ...
        "summary": {
            ...
            "query_cache_entries": int(cache.get("entries") or 0),
            "query_cache_hits": int(cache.get("hits") or 0),
        },
        "artifacts": {
            "ledger_db_path": LEDGER_DB_PATH,
            "derived_index_path": DERIVED_INDEX_PATH,
            "query_cache_path": QUERY_CACHE_PATH,
            "rebuild_proof_path": REBUILD_PROOF_PATH,
        },
        "ledger_before": ledger_before.get("summary", {}),
        "ledger_after": ledger_after.get("summary", {}),
        "derived_index": derived.get("summary", {}),
        "lineage": dict(derived.get("lineage") or {}),
