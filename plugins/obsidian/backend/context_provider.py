...
def retrieve_vault_context(owner: Optional[str], query: str, budget: int, mode: str = "chat") -> Dict[str, Any]:
    ...
    # Semantic fallback can write/update the embedding cache, so keep it behind
    # the hybrid retrieval feature flag for the read-only MVP.
    keyword_scored_paths = {n["path"] for n in notes}
    if is_enabled("obsidian_hybrid_retrieval_enabled") and query_terms and len(notes) < 5:
        try:
