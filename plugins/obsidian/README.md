...
- Browser smoke verification is partially complete: the standalone app shell loads with a CSP-compatible external bootstrap, plugin web assets return 200, unauthenticated data routes remain 401, a real browser-login smoke loads vault files through an authenticated session, and isolated test-vault smokes verify visible Cytoscape graph layout, focused graph refresh after selecting a note from the tree, visible current-node behavior, and graph filter classes. The full-app graph-filter smoke is still a release-candidate risk and must pass in a fresh browser harness before tagging or publishing an RC.
