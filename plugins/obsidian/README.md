...
- Browser smoke verification is partially complete: the standalone app shell loads with a CSP-compatible external bootstrap, plugin web assets return 200, unauthenticated data routes remain 401, a real browser-login smoke loads vault files through an authenticated session, and isolated test-vault smokes verify visible Cytoscape graph layout plus focused graph refresh after selecting a note from the tree. Visual current-node marking and graph filters still require browser smoke verification before tagging a release candidate.
