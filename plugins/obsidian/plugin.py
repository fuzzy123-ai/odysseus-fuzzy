...
async def handle_memory_tree_status(content: str, owner: Optional[str] = None, **kwargs) -> dict:
    """Returns read-only SOMT index health and branch overview."""
    try:
        vault_dir = get_unlocked_vault_path_by_owner(owner)
        return {"output": json.dumps(memory_tree_status(vault_dir), ensure_ascii=False, indent=2), "exit_code": 0}
    except Exception as e:
...
async def handle_memory_tree_analyze(content: str, owner: Optional[str] = None, **kwargs) -> dict:
    ...
    try:
        params = json.loads((content or "{}").strip() or "{}")
        vault_dir = get_unlocked_vault_path_by_owner(owner)
        return {"output": json.dumps(analyze_memory_tree(vault_dir, limit=params.get("limit")), ensure_ascii=False, indent=2), "exit_code": 0}
    except Exception as e:
        return {"error": f"Failed to analyze memory tree: {e}", "exit_code": 1}


async def handle_knowledge_audit(content: str, owner: Optional[str] = None, **kwargs) -> dict:
    """Runs the read-only Freshness Gate audit."""
    try:
        vault_dir = get_unlocked_vault_path_by_owner(owner)
        return {"output": json.dumps(audit_knowledge(vault_dir), ensure_ascii=False, indent=2), "exit_code": 0}
    except Exception as e:
        return {"error": f"Failed to audit knowledge freshness: {e}", "exit_code": 1}


async def handle_quarantine_list(content: str, owner: Optional[str] = None, **kwargs) -> dict:
    """Lists knowledge isolated from default retrieval."""
    try:
        vault_dir = get_unlocked_vault_path_by_owner(owner)
        return {"output": json.dumps(quarantine_list(vault_dir), ensure_ascii=False, indent=2), "exit_code": 0}
    except Exception as e:
        return {"error": f"Failed to list quarantined knowledge: {e}", "exit_code": 1}
...
def setup(ctx):
    ...
    tools = [
        ...
        _tool_spec("obsidian_memory_status", "Return unified read-only memory readiness with readiness_gate, freshness_isolation_flags, and raptor_lineage_flags across SOMT, Freshness Gate, quarantine, and RAPTOR.", {}, [], handle_memory_status),
