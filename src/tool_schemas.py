...
FUNCTION_TOOL_SCHEMAS = [
    ...
    },
    {
        ...
        "function": {
            "name": "app_api",
            "description": "Generic loopback to allowed internal Odysseus endpoints. Use this when there's no named tool for what the user wants. Hits safe UI/API routes for cookbook read state, gallery/library reads, settings reads, research status, compare, etc. action='endpoints' returns the OpenAPI surface (use `filter` to narrow). action='call' (default) takes method+path+body. Sensitive auth/user/admin/shell paths, host-control Cookbook mutation routes, direct task/session/document/research/gallery mutation routes, and admin mutations are blocked for safety. Gallery routes are read/list only through app_api. Use named tools such as manage_tasks, manage_session, manage_documents, manage_research, manage_settings, manage_endpoints, manage_webhooks, manage_mcp, and list_email_accounts when available.",
            "parameters": {
                "type": "object",
                "properties": {
