...
FUNCTION_TOOL_SCHEMAS = [
    ...
    },
    {
        ...
        "function": {
            "name": "app_api",
            "description": "Generic loopback to allowed internal Odysseus endpoints. Use this when there's no named tool for what the user wants. Hits safe UI/API routes for contacts reads, cookbook read state, email reads, gallery/library reads, memory reads, notes/calendar reads, prefs reads, settings reads, research status, compare, etc. action='endpoints' returns the OpenAPI surface (use `filter` to narrow). action='call' (default) takes method+path+body. Sensitive auth/user/admin/shell paths, host-control Cookbook mutation routes, direct task/session/document/research/contacts/email/gallery/memory/notes/calendar/prefs mutation routes, and admin mutations are blocked for safety. Contacts, email, gallery, memory, notes, calendar, and prefs routes are read/list only through app_api. Use named tools such as list_email_accounts, list_emails, read_email, send_email, reply_to_email, bulk_email, archive_email, delete_email, mark_email_read, resolve_contact, manage_contact, manage_memory, manage_notes, manage_calendar, manage_tasks, manage_session, manage_documents, manage_research, manage_settings, manage_endpoints, manage_webhooks, manage_mcp when available; use ui_control for themes and UI prefs.",
            "parameters": {
                "type": "object",
                "properties": {
