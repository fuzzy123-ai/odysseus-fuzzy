...
FUNCTION_TOOL_SCHEMAS = [
    ...
    },
    {
        ...
        "function": {
            "name": "manage_repos",
            "description": "Manage explicitly registered repositories, read Git facts, and run gated local commit/push/forge metadata flows. Read actions list/get/status/log/diff_stat/changed_paths/remotes/commit_plan/push_plan/forge_plan need no confirmation. Mutations register/forget/update_policy, commit, push, and live forge_metadata require confirmed=true. commit also requires exact changed_paths, checks_passed=true, and content_reviewed=true. push also requires remote policy approval, operator_go=true, live_enabled=true, matching branch_name, and matching commit_sha. forge_metadata requires secure auth readiness, operator_go=true, and live_enabled=true; never pass tokens in chat. Never reset, merge, delete repo files, or mutate Git history outside the gated runners.",
            "parameters": {
                "type": "object",
                "properties": {
                    "action": {"type": "string", "enum": ["list", "get", "status", "log", "diff_stat", "changed_paths", "remotes", "commit_plan", "commit", "push_plan", "push", "forge_plan", "forge_metadata", "register", "forget", "update_policy"], "description": "list shows registered repos; get shows one registry record; status/log/diff_stat/changed_paths/remotes read Git facts; commit_plan explains a gated local commit; commit stages exact reviewed paths and commits them when all gates pass; push_plan explains a gated push; push runs only through remote policy and live gates; forge_plan explains GitHub/Gitea/Forgejo metadata gates; forge_metadata runs only through secure auth and live gates; register/forget/update_policy mutate only the registry."},
                    "repo_id": {"type": "string", "description": "Repo id for get/status/log/diff_stat/changed_paths/remotes/commit_plan/commit/push_plan/push/forge_plan/forge_metadata/forget/update_policy; optional for register when title/path can produce one."},
