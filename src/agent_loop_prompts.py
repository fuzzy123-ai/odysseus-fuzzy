...
- Notes/reminders use `manage_notes`; todo/checklist item mutations use `manage_todos`, never memory. Recurring requests use `manage_tasks`.
...
- Calendar: call `manage_calendar` with `action=list_calendars` FIRST before create/update/delete operations.
- Scope/assumption preface: when the user asks to create a new project, plan a new roadmap, or execute/start a roadmap, begin with exactly two short lines: "Need to know:" and "Assumptions:". List only information that would materially change the plan, then proceed if it is non-blocking. If it is blocking because it affects live writes, destructive actions, credentials, cost, security, or ownership, ask one concise question and stop. Do not use this preface for trivial notes, todos, reminders, calendar items, or simple one-off tasks.
- Notes/reminders use `manage_notes`; todo/checklist item mutations use `manage_todos`, never memory. `manage_tasks` is for recurring background work.
...
- Notes, reminders, and checklist-container discovery use `manage_notes`, not memory.
- Todo/checklist item mutations use `manage_todos`, never `manage_notes` or memory.
...
- Recurring/automatic/scheduled requests create a `manage_tasks` task; do not just perform the action once.""",
    "todos": """\
## Todo rules
- Todo/checklist item mutations use `manage_todos`, never `manage_memory` or `manage_notes`.
- `manage_notes` is for notes, reminders, and checklist discovery only.
- Use full list_ref/item_ref values; remove requires confirmed=true.""",
    "ui": """\
## UI rules
...
_DOMAIN_TOOL_MAP = {
    ...
    "todos": {"manage_todos", "manage_notes"},
    "ui": {"ui_control"},
...
```
Notes, reminders, and checklist containers. Use this for "create/add/write a note", "create a checklist", and "remind me to X at <time>" — never use memory for note content. Use `manage_todos` for all checklist-item mutations. For reminders, pair a short `title` (what to do) with a `due_date` (when). `due_date` accepts natural language ("tomorrow at 1pm", "in 2 hours", "next monday 9am") or ISO ("2026-05-12T13:00:00"). Actions here: `list`, `add` (title, content, note_type, color, label, due_date), `update`, `delete`. Delete requires explicit user confirmation and `confirmed=true`.""",
    "manage_todos": "- ```manage_todos``` — Mutate items in an existing active checklist. JSON: {\"action\":\"list|add|complete|reopen|remove\",\"list_ref\":\"full-note-id\",\"item_ref\":\"...\",\"text\":\"...\",\"idempotency_key\":\"...\",\"confirmed\":true}. add needs text+idempotency_key; item actions use exactly one selector; remove requires confirmed=true. Notes/reminders/discovery remain manage_notes.",
