...
Generate an image. Line 1 = description, line 2 = model name, line 3 = WxH (e.g. 1024x1024), line 4 = quality.""",
    ...
    "manage_github_issues": "- ```manage_github_issues``` - GitHub Issue Intelligence. Args (JSON): {\"action\":\"sync|duplicate_search|create_triaged|set_fields\", \"repository\":\"owner/repo\", \"title\":\"...\", \"fields\":{...}, \"confirmed\":true}. `duplicate_search` is local/read-only over already-synced issue records and returns top duplicate candidates with score/reason. `sync` is live-read gated. `create_triaged` and `set_fields` are write-like: they require confirmed=true and future live/auth gates; without those gates they return a safe blocker/plan. Never pass GitHub tokens in chat.",
    "manage_notes": """\
```manage_notes
