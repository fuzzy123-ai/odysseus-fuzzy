...
Generate an image. Line 1 = description, line 2 = model name, line 3 = WxH (e.g. 1024x1024), line 4 = quality.""",
    ...
    "manage_tasks": "- ```manage_tasks``` — Create and manage scheduled background tasks (recurring AI jobs). Args (JSON): {\"action\": \"list|create|edit|delete|pause|resume|run\", ...}. For the prepared morning todo digest use {\"task_type\":\"action\",\"action_name\":\"todo_digest\",\"schedule\":\"daily\",\"scheduled_time\":\"HH:MM\",\"output_target\":\"telegram\"} when the user asks for Telegram delivery; scheduled_time is UTC, so convert from the user's stated timezone. Never include Telegram chat IDs or tokens. Delete requires explicit user confirmation and confirmed=true.",
