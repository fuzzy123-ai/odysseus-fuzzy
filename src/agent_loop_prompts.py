...
- "Create/add/write a note" / "notes" / "remind me to X at <time>" → use `manage_notes`. For reminders, include a `due_date`.
- Todo/task/checklist item list/add/complete/reopen/remove → use `manage_todos`, with stable refs from list and an idempotency key for mutations. Never store Todo state in `manage_memory`.
...
- Calendar: call `manage_calendar` with `action=list_calendars` FIRST before create/update/delete operations.
- Scope/assumption preface: when the user asks to create a new project, plan a new roadmap, or execute/start a roadmap, begin with exactly two short lines: "Need to know:" and "Assumptions:". List only information that would materially change the plan, then proceed if it is non-blocking. If it is blocking because it affects live writes, destructive actions, credentials, cost, security, or ownership, ask one concise question and stop. Do not use this preface for trivial notes, todos, reminders, calendar items, or simple one-off tasks.
- "Create/add/write a note" / "notes" / "remind me to X at <time>" → use `manage_notes`; include a `due_date` for reminders. `manage_tasks` is for RECURRING background AI jobs, NOT one-off reminders.
- Todo/task/checklist item list/add/complete/reopen/remove → use `manage_todos`, never `manage_memory` or `manage_notes`.
...
- Freeform notes and reminder notes use `manage_notes`; canonical Todo item operations use `manage_todos`.
- Clear Todo intents must not use `manage_memory`. Stable refs and mutation idempotency keys are mandatory when available.
...
_DOMAIN_TOOL_MAP = {
    ...
    "notes_calendar_tasks": {"manage_notes", "manage_todos", "manage_calendar", "manage_tasks"},
    "ui": {"ui_control"},
...
```
Freeform notes and user reminders. Use `manage_todos` for Todo item operations. For reminders, pair a short `title` with a `due_date`. Actions remain `list`, `add`, `update`, `delete`, `toggle_item` for legacy/UI compatibility. Delete requires explicit user confirmation and `confirmed=true`.""",
