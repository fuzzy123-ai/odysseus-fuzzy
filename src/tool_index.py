...
BUILTIN_TOOL_DESCRIPTIONS: Dict[str, str] = {
    ...
    "query_knowledge": "Query owner-scoped Unified Source Index evidence for code, documents, or memory with bounded domain, mode, scope, budget, and classification. It is read-only; use read_file or a domain reader for exact source content.",
    ...
    "publish_artifact": "Publish a generated .py, self-contained .html, or PNG workspace file as an owner-scoped chat attachment with a real download ID. For screenshots, request vision inspection before making a visual-quality claim. Publishing proves download readiness, not interactive native-GUI execution.",
    "verify_pygame_headless": "Run bounded dummy-SDL verification for a generated Pygame .py file: syntax, pygame import, capped frame run, and PNG capture. This can prove headless testing but never a visible interactive preview or visual quality; publish and inspect the outputs separately.",
    "commit_project": "Create one reviewed local project commit with title and description, retain it in the local Forge, and queue synchronization according to the stored project policy. Never choose Nextcloud or GitHub in the tool call; provider selection comes only from project settings.",
    ...
    "manage_tokens": "API token management: list, create, update/rename, or delete API access tokens through the token routes. create/update/delete require confirmed=true; newly created token values are shown once.",
    "manage_presets": "Preset management: list presets/templates/groups, update the custom preset, save/delete user templates, or save group presets through the same Presets UI routes. Writes and deletes require confirmed=true.",
    "manage_personal_docs": "Personal Docs / RAG source management: list indexed files/directories, reload the personal docs index, add/remove source directories, or delete/exclude a file through the same Personal Docs UI routes. reload/add/remove/delete require confirmed=true; upload stays UI-only.",
    "manage_embeddings": "Embedding model management: list embedding models, check model download status, read endpoint status, download or delete local FastEmbed model caches, and clear the custom embedding endpoint through the Embedding Settings routes. download/delete/clear_endpoint require confirmed=true; set_endpoint stays UI/secure-handoff-only because it performs a live health check and may require an API key.",
    "manage_assistant": "Personal assistant management: read assistant session/settings/timezones/run status, update assistant name/personality/model/tools/timezone/check-ins, or manually run a check-in through the same Assistant UI routes. update/run require confirmed=true; endpoint_url stays UI/manage_endpoints-only.",
    "manage_plugins": "Admin plugin manager: list installed plugins, read registry/registries/status, enable/disable/reload/rescan, install registry plugin ids, uninstall plugins, and add/remove plugin registries through the same Plugins UI routes. Mutations require confirmed=true; direct ZIP URL installs and plugin-specific provider actions stay UI/provider-specific.",
    ...
    "manage_settings": "Change real app settings, feature flags, and tool toggles through the policy-aware settings service. Supports get/list/explain/set/reset/patch with scope auto|user|global, features with confirmed=true, request_secret/secret_handoffs for secure API-key handoff (never pass the secret value), and disable_tool/enable_tool/list_tools. Use for any 'change my.../set my.../use X for.../turn on...' preference request.",
    "recent_changes": "Local Odysseus patch-note history. Use for Neuerungen, Patch Notes, changelog, what changed, recent changes, what changed today/yesterday/in the last N hours. Collects commits, dirty tracked files, untracked files, and recently modified files, then stores deduped snapshots for later UI history.",
    "manage_repos": "Registered repository management and intelligence. Use for registry list/get/update, read-only Git facts, sanitized repo change history, and commit_plan/push_plan/forge_plan previews. It never commits or performs provider delivery. Use commit_project once for the sole reviewed local commit and policy-selected provider synchronization workflow.",
    "manage_github_issues": "GitHub Issue Intelligence. Use for duplicate issue preview, triaged issue planning, GitHub Issue Fields mapping, issue labels/priority/effort/status fields, and bounded read-only provider sync. duplicate_search runs locally on synced IssueRecord rows and returns top candidates with score/reason; sync reads GitHub only when confirmed and server-side live env plus repository allowlist are enabled, then writes local IssueRecord rows only; create_triaged and set_fields are write-like and require confirmed=true plus future live/auth gates. Never pass GitHub tokens in chat.",
    "manage_nextcloud_transfer": "Universal Inbox Nextcloud transfer tool. Use for Nextcloud WebDAV readiness, safe smoke planning, and copy-only Universal Inbox writes. readiness checks server-side env gates without showing secrets or probing the network; smoke_plan prepares a harmless test transfer without writing; execute writes only when review_approved/confirmed=true, operator_live_go=true, live write env gates are enabled, and a server-side WebDAV client can be built. Never ask for or echo Nextcloud credentials in chat; deletes, moves and overwrites stay blocked.",
    ...
    "ask_user": "Ask the user for material clarification instead of guessing. For simple choices, provide legacy {question, options, multi}; it is normalized to odysseus.clarification_request.v2. For project/coding intake, prefer v2 with scope, intent_summary, questions[], batch, required flags, reasons, dependencies and visible defaults. Calling this ENDS your turn. Do not ask for secrets, tokens, chat IDs, raw provider output or private host paths, and do not use it for irreversible-action confirmation if a dedicated flow exists.",
    ...
    "manage_notes": "Create and manage notes, reminders, and checklist containers (Google Keep-style). Use this for note/reminder creation and finding or creating a checklist container — NEVER hit /api/notes via app_api. Accepts natural-language `due_date` like 'tomorrow at 9am' or '11pm today' (parsed in the USER'S timezone). The due_date IS the reminder — it fires a notification at that time, so do NOT also create a calendar event for the same reminder. Set colors, labels, pin, archive. Do NOT use manage_memory for note content.",
    "manage_todos": "Manage items in an existing active checklist: list, idempotent add, complete, reopen, or confirmed remove. Use full list and item IDs; never use prefixes. For notes, reminders, and checklist discovery use manage_notes. Never use manage_memory for todo mutations.",
    ...
    "app_api": "Generic loopback to allowed Odysseus internal endpoints. Use this when the user wants something the UI can do but there's no named tool for it. Use for safe read/list/status UI/API routes such as assistant reads, chat status reads, cleanup preview, Codex plugin reads, compare history, contacts reads, cookbook GPUs/state, document reads, email reads, embedding status reads, gallery/library reads, memory reads, notes/calendar reads, personal document reads, plugin reads, preset reads, prefs reads, repo reads/status/changes, research status, search config/provider reads, skills reads, and other non-destructive endpoints. Sensitive auth/user/admin/shell/upload/signature/editor-draft paths, host-control Cookbook mutations, admin mutations, direct task/session/chat/rewrite/context-injection/document/research/assistant/codex/compare/contacts/email/embeddings/gallery/memory/notes/calendar/personal/plugins/presets/prefs/skills mutation routes, repo register/policy/commit-plan/push-plan routes, cleanup execution, search execution, and email account discovery are blocked; assistant, Codex plugin, compare, contacts, documents, email, embeddings, gallery, memory, notes, calendar, personal-docs, plugin-manager/provider, preset, prefs, repos, skills, and chat-run routes are read/list/status only through app_api. Upload attachment, saved visual signature, and gallery editor draft reads/writes are blocked entirely through app_api. Use named tools such as list_email_accounts, list_emails, read_email, send_email, reply_to_email, bulk_email, archive_email, delete_email, mark_email_read, resolve_contact, manage_contact, manage_memory, manage_notes, manage_calendar, manage_tasks, manage_session, manage_documents, manage_research, manage_settings, manage_skills, manage_endpoints, manage_webhooks, manage_mcp, manage_presets, manage_personal_docs, manage_embeddings, manage_assistant, manage_plugins, manage_repos, and create_session/list_sessions when available. action='endpoints' with filter=<keyword> lists available endpoints; action='call' takes method+path+body. Assistant settings and check-in runs use manage_assistant; chat/rewrite/context injection uses the normal chat UI or manage_session; cleanup execution uses manage_session or the Cleanup UI; Codex plugin mutations use the native named tools; compare start/vote/record/delete uses the Compare UI or chat_with_model; document create/import/export/mutations use document tools or manage_documents; embedding model/cache changes and endpoint clears use manage_embeddings, while endpoint set stays Embedding Settings UI/secure-handoff-only; repo register/policy/commit-plan/push-plan routes use manage_repos or the Repo/Project UI; search execution uses web_search/web_fetch/trigger_research; personal document/RAG source changes use manage_personal_docs; plugin manager changes use manage_plugins, while plugin-specific provider actions stay Plugins UI/provider-specific; preset writes use manage_presets; uploads use the attachment UI, signatures use the Signature/Documents UI, editor drafts use the Gallery Editor UI, not app_api; themes and UI prefs use ui_control, not app_api; registered settings use manage_settings.",
    ...
    _KEYWORD_HINTS = {
        **dict(keyword_hint_pairs()),
        ...
        frozenset({"background job", "background jobs", "bg job", "bg jobs",
            ...
            {"manage_bg_jobs"},
        frozenset({"todo", "todos", "to-do", "to-dos", "todolist", "todo list", "task list", "checklist", "checklists", "checkliste", "checklisten", "aufgabe", "aufgaben", "cheklist", "toodo"}):
            {"manage_todos"},
        frozenset({"note", "notes", "reminder", "remind", "remember to"}):
            {"manage_notes"},
    ...
    def get_tools_for_query(
        ...
        for keywords, tools in self._KEYWORD_HINTS.items():
            if any(re.search(rf"\b{re.escape(kw)}\b", ql) for kw in keywords):
                base.update(tools)
        if re.search(r"\b(?:todo|todos|to-do|to-dos|todolist|todo list|task list|checklist|checklists|checkliste|checklisten|aufgabe|aufgaben|cheklist|toodo)\b", ql):
            base.add("manage_todos")
