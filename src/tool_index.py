...
BUILTIN_TOOL_DESCRIPTIONS: Dict[str, str] = {
    ...
    "app_api": "Generic loopback to allowed Odysseus internal endpoints. Use this when the user wants something the UI can do but there's no named tool for it. Use for safe read/list/status UI/API routes such as contacts reads, cookbook GPUs/state, email reads, gallery/library reads, memory reads, notes/calendar reads, prefs reads, research status, skills reads, compare, and other non-destructive endpoints. Sensitive auth/user/admin/shell paths, host-control Cookbook mutations, admin mutations, direct task/session/document/research/contacts/email/gallery/memory/notes/calendar/prefs/skills mutation routes, and email account discovery are blocked; contacts, email, gallery, memory, notes, calendar, prefs, and skills are read/list only through app_api. Use named tools such as list_email_accounts, list_emails, read_email, send_email, reply_to_email, bulk_email, archive_email, delete_email, mark_email_read, resolve_contact, manage_contact, manage_memory, manage_notes, manage_calendar, manage_tasks, manage_session, manage_documents, manage_research, manage_settings, manage_skills, manage_endpoints, manage_webhooks, manage_mcp, and create_session/list_sessions when available. action='endpoints' with filter=<keyword> lists available endpoints; action='call' takes method+path+body. Themes and UI prefs use ui_control, not app_api; registered settings use manage_settings.",
