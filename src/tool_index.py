...
BUILTIN_TOOL_DESCRIPTIONS: Dict[str, str] = {
    ...
    "commit_project": "Commit an explicitly reviewed set of project files after required checks pass. Use only with the project commit workflow and its path-scoped confirmation contract.",
    ...
    "manage_repos": "Registered repository management and intelligence. Use for repo registry list/get, read-only Git status, recent commits, diff stat, changed paths, redacted remotes, repo-scoped changes/change_history capsules for Project Context/Memory/RaptorGraph, gated commit_plan/commit/push_plan/push, and forge_plan/forge_metadata for GitHub/Gitea/Forgejo metadata. changes answers 'what is new in repo X' with sanitized snapshots: public may summarize paths/commit topics, private stays local-only, sensitive exposes only redacted metadata and never raw diffs. commit requires confirmed=true, exact changed_paths, checks_passed=true, and content_reviewed=true. push requires confirmed=true, operator_go=true, live_enabled=true, matching branch_name, matching commit_sha, and remote policy approval. forge_metadata requires confirmed=true, auth_ready=true from secure handoff or server credentials, operator_go=true, and live_enabled=true. Never pass tokens in chat, reset, merge, delete repo files, or bypass the runners.",
    ...
    "manage_notes": "Create and manage freeform notes and reminder notes (Google Keep-style). Use manage_todos for Todo item list/add/complete/reopen/remove. NEVER hit /api/notes via app_api. Accepts natural-language due_date parsed in the user's timezone. Do not use manage_memory for note content.",
