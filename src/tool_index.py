...
BUILTIN_TOOL_DESCRIPTIONS: Dict[str, str] = {
    ...
    "app_api": "Generic loopback to allowed Odysseus internal endpoints. Use this when the user wants something the UI can do but there's no named tool for it. Use for safe read/list/status UI/API routes such as cookbook GPUs/state, gallery/library reads, research status, compare, and other non-destructive endpoints. Sensitive auth/user/admin/shell paths, host-control Cookbook mutations, admin mutations, direct task/session/document/research/gallery mutation routes, and email account discovery are blocked; gallery is read/list only through app_api. Use named tools such as manage_tasks, manage_session, manage_documents, manage_research, manage_settings, manage_endpoints, manage_webhooks, manage_mcp, list_email_accounts, and create_session/list_sessions when available. action='endpoints' with filter=<keyword> lists available endpoints; action='call' takes method+path+body. Themes use ui_control, not app_api.",
