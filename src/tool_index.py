...
BUILTIN_TOOL_DESCRIPTIONS: Dict[str, str] = {
    ...
    "manage_repos": "Registered repository management and intelligence. Use for repo registry list/get, confirmed register/forget/update_policy, read-only Git status, recent commits, diff stat, changed paths, redacted remotes, gated commit_plan/commit/push_plan/push, and forge_plan/forge_metadata for GitHub/Gitea/Forgejo metadata. commit requires confirmed=true, exact changed_paths, checks_passed=true, and content_reviewed=true. push requires confirmed=true, operator_go=true, live_enabled=true, matching branch_name, matching commit_sha, and remote policy approval. forge_metadata requires confirmed=true, auth_ready=true from secure handoff or server credentials, operator_go=true, and live_enabled=true. Never pass tokens in chat, reset, merge, delete repo files, or bypass the runners.",
