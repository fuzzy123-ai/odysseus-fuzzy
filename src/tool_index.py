...
BUILTIN_TOOL_DESCRIPTIONS: Dict[str, str] = {
    ...
    "manage_github_issues": "GitHub Issue Intelligence. Use for duplicate issue preview, triaged issue planning, GitHub Issue Fields mapping, issue labels/priority/effort/status fields, and read-only sync planning. duplicate_search runs locally on synced IssueRecord rows and returns top candidates with score/reason; sync returns a live read gate; create_triaged and set_fields are write-like and require confirmed=true plus future live/auth gates. Never pass GitHub tokens in chat.",
