...
- Do not use shell for repo intelligence when `manage_repos` can answer it. `manage_repos` never stages, commits, pushes, resets, merges, or deletes repo files.""",
}
...
Generate an image. Line 1 = description, line 2 = model name, line 3 = WxH (e.g. 1024x1024), line 4 = quality.""",
    ...
    "manage_repos": "- ```manage_repos``` — Registered repo management and Git intelligence. Args (JSON): {\"action\":\"list|get|status|log|diff_stat|changed_paths|remotes|register|forget|update_policy\", \"repo_id\":\"...\", \"confirmed\":true}. Read actions need no confirmation. `register`, `forget`, and `update_policy` require confirmed=true; registering outside allowed roots also requires operator_go=true. Never use it to stage, commit, push, reset, merge, or delete repo files.",
    "manage_notes": """\
```manage_notes
