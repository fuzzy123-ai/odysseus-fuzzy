...
Generate an image. Line 1 = description, line 2 = model name, line 3 = WxH (e.g. 1024x1024), line 4 = quality.""",
    ...
    "manage_repos": "- ```manage_repos``` — Registered repo management and Git intelligence. Args (JSON): {\"action\":\"list|get|status|log|diff_stat|changed_paths|remotes|commit_plan|commit|push_plan|push|register|forget|update_policy\", \"repo_id\":\"...\", \"confirmed\":true}. Read actions plus `commit_plan`/`push_plan` need no confirmation. `register`, `forget`, `update_policy`, `commit`, and `push` require confirmed=true. commit also requires exact changed_paths, checks_passed=true, and content_reviewed=true. push also requires operator_go=true, live_enabled=true, branch_name, commit_sha, and remote policy approval. Never use it to reset, merge, or delete repo files.",
    "manage_notes": """\
```manage_notes
