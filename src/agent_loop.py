...
- Do not use shell for repo intelligence when `manage_repos` can answer it. Local commits must go through `commit_plan`/`commit` with confirmed exact paths, green checks, and content review. It never pushes, resets, merges, or deletes repo files.""",
}
...
Generate an image. Line 1 = description, line 2 = model name, line 3 = WxH (e.g. 1024x1024), line 4 = quality.""",
    ...
    "manage_repos": "- ```manage_repos``` — Registered repo management and Git intelligence. Args (JSON): {\"action\":\"list|get|status|log|diff_stat|changed_paths|remotes|commit_plan|commit|register|forget|update_policy\", \"repo_id\":\"...\", \"confirmed\":true}. Read actions and `commit_plan` need no confirmation. `register`, `forget`, `update_policy`, and `commit` require confirmed=true; commit also requires exact changed_paths, checks_passed=true, and content_reviewed=true. Never use it to push, reset, merge, or delete repo files.",
    "manage_notes": """\
```manage_notes
