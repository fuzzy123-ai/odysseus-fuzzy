...
def build_default_agent_team_card_payload() -> AgentTeamCardApiPayload:
    ...
    return build_agent_team_card_payload(team_card)


def build_agent_team_card_payload(team_card: AgentTeamCard) -> AgentTeamCardApiPayload:
    ...
    return AgentTeamCardApiPayload(
        team=tuple(_visible_profile_payload(profile) for profile in team_card.visible_agents),
        hidden_workers=tuple(_hidden_worker_payload(profile) for profile in team_card.hidden_workers),
...
def _visible_profile_payload(profile: AgentProfile) -> dict[str, Any]:
    return {
...
def _hidden_worker_payload(profile: AgentProfile) -> dict[str, Any]:
    return {
        "agent_id": profile.agent_id,
        "display_name": _sanitize_text(profile.display_name),
        "role_preset_id": profile.role_preset_id,
