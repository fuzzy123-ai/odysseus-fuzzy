...
def current_manual_evidence_entries() -> tuple[ManualEvidenceEntry, ...]:
    ...
    return (
        ...
        ManualEvidenceEntry(
            ...
            notes=(
                "Synthetic gitignored workspace data vault with two harmless markdown files.",
                "Exported 2 files, imported 2 files, rebuild proof configured, query layer ready, 1 citation.",
                "No production vault, source write, data loss, secrets, or raw provider output recorded.",
            ),
        ),
        ManualEvidenceEntry(
            "known-limits-review",
