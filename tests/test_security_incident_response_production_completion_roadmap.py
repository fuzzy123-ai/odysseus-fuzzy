...
def test_sirp12_observe_packet_is_consumed_exactly_once_and_projection_is_bounded() -> None:
    ...
    assert set(live_go_by_id) == {
        ...
        "SEC202-EFFECTIVE-RECEIPT-CLASS-DIAGNOSTIC-20260828",
        "SEC203-EXACT-UNKNOWN-RECEIPT-RECOVERY-20260828",
        "SEC204-FRESH-POST-RECOVERY-BACKUP-AND-SNAPSHOT-VERIFY-20260828",
        }
    ...
    assert sec199["status"] == "used_completed_receipt_mode_mismatch_diagnosed"
    assert sec199["consumption_status"] == "consumed_terminal_read_only_diagnostic_completed"
    assert sec199["consumed"] is True
    assert sec199["limits"]["maximum_invocations"] == 1
    assert sec199["limits"]["outer_timeout_seconds"] == 30
    assert sec199["limits"]["retries"] == 0
    assert sec199["invocation_counter"] == 1
    assert sec199["result_counter"] == 1
    assert sec199["external_action_executed"] is True
    assert sec199["terminal_result"]["status"] == "ok"
    assert sec199["terminal_result"]["receipt_matches"] is False
    assert sec199["terminal_result"]["recovery_preflight_ready"] is False
    assert sec199["terminal_result"]["evidence_sha256"] == (
    ...
    assert sec200["limits"]["retries"] == 0
    ...
    assert sec201["status"] == "used_completed_effective_receipt_mode_mismatch_diagnosed"
    assert sec201["consumption_status"] == "consumed_terminal_read_only_diagnostic_completed"
    assert sec201["consumed"] is True
    ...
    assert sec201["invocation_counter"] == 1
    assert sec201["result_counter"] == 1
    assert sec201["external_action_executed"] is True
    assert sec201["terminal_result"]["status"] == "ok"
    assert sec201["terminal_result"]["receipt_file_valid"] is False
    ...
    assert sec202["limits"]["retries"] == 0
    assert sec202["current_incident_observation_authority"] is True
