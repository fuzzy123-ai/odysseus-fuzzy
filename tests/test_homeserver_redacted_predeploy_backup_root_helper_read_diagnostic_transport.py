...
from ops.homeserver import redacted_predeploy_backup_root_helper_read_diagnostic_transport as transport
...
def _observed():
    return diagnostic.envelope("observed", "preexec_failed", locked=True, bound=True, invoked=True)
...
def test_pin_matches_source_and_default_is_inert() -> None:
    ...
    assert transport.collect_published_root_helper_read_diagnostic()["error_code"] == "execution_disabled"
...
def test_transport_sends_only_pinned_source_and_accepts_fixed_envelope() -> None:
    ...
    value = transport.collect_published_root_helper_read_diagnostic(execute=True, runner=runner)
...
def test_predispatch_mismatch_blocks_and_postdispatch_ambiguity_is_unknown() -> None:
    value = transport.collect_published_root_helper_read_diagnostic(execute=True, runner=lambda command, **kwargs: SimpleNamespace(returncode=0, stdout=b"wrong"))
