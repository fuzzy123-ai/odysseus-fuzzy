...
@pytest.mark.asyncio
async def test_delegate_injects_provider_warning_context(monkeypatch, tmp_path):
    monkeypatch.setenv("OBSIDIAN_VAULT_DIR", str(tmp_path))
    captured = {}
    ...
    async def fake_llm_call_async(url, model, messages, **kwargs):
        ...
        return json.dumps({
            "status": "done",
            "summary": "Worker saw the warning.",
            "findings": [],
            "suggested_next_step": "Unlock the vault.",
        })

    monkeypatch.setattr("src.llm_core.llm_call_async", fake_llm_call_async)
    try:
        result = await do_delegate(
            json.dumps({"task": "Inspect warning path.", "context_query": "vault"}),
            endpoint_url="http://llm.test/v1",
            model="test-model",
            headers={},
            owner="alice",
    ...
    assert result["status"] == "done"
    assert any(msg["content"].startswith("Provider warnings") for msg in captured["messages"])
    assert any("test.delegate_context: vault locked" in msg["content"] for msg in captured["messages"])
