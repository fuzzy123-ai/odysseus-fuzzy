...
def test_status_is_read_only_and_uses_cookbook_native_tool():
    ...
    assert plan["schema"] == COOKBOOK_CONTROL_SCHEMA
    assert plan["args"] == {}
...
def test_serve_requires_operator_and_live_go_before_confirmed_args():
    ...
    assert waiting["args"]["name"] == "gemma3-4b-maintenance"
    ...
    assert ready["args"]["maintenance_model_ref"] == "gemma3:4b"
...
def test_stop_and_adopt_are_gated_and_safe():
    ...
    assert adopt["args"]["model"] == "gemma3:4b"
