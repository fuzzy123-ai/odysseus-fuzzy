...
if ($DisableBuiltinMcp) {
    ...
    Write-Host "Built-in MCP auto-start is disabled. Use -EnableBuiltinMcp to opt back in."
} else {
    Remove-Item Env:ODYSSEUS_DISABLE_MCP -ErrorAction SilentlyContinue
    Write-Host "Built-in MCP auto-start is enabled (default). Use -DisableBuiltinMcp to opt out."
}
