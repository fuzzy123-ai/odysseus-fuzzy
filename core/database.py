...
def init_db():
    """
    Initialize the database by creating all tables and running idempotent
    migrations. Migration implementations live in ``core.database_migrations``
    so this module can stay focused on schema models and public DB helpers.
    """
    from core.database_migrations import run_database_migrations

    run_database_migrations()


def __getattr__(name: str):
    """Compatibility access for legacy private migration helpers."""
    if name.startswith("_migrate_"):
        from core import database_migrations

        return getattr(database_migrations, name)
