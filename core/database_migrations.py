"""Startup database migrations for :mod:`core.database`.

The SQLAlchemy models and public database helpers stay in ``core.database``;
this module owns the idempotent startup migrations.  Functions refresh their
context from ``core.database`` before running so tests that monkeypatch
``DATABASE_URL`` or ``engine`` on the canonical module keep working.
"""
...
def _refresh_database_context() -> None:
    db = sys.modules.get("core.database")
    if db is None:
        from core import database as db

    globals().update(
        AUTH_FILE=db.AUTH_FILE,
        Base=db.Base,
        DATABASE_URL=db.DATABASE_URL,
        MEMORY_FILE=db.MEMORY_FILE,
        SETTINGS_FILE=db.SETTINGS_FILE,
        USER_PREFS_FILE=db.USER_PREFS_FILE,
        engine=db.engine,
        utcnow_naive=db.utcnow_naive,
    )


def _with_database_context(fn):
    @wraps(fn)
    def wrapper(*args, **kwargs):
        _refresh_database_context()
...
def run_database_migrations() -> None:
    """Run the startup migration sequence used by ``core.database.init_db``."""
    _refresh_database_context()
