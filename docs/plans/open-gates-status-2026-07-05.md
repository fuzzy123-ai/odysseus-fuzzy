...
| 90 | GitHub issue intelligence | Read-only live smoke completed | Local duplicate/write-plan contracts, HTTP readiness, duplicate preview and write gate behavior are live-verified; bounded provider sync adapter is implemented behind server env, repo allowlist and confirmation. | Bounded provider sync needs server-side env activation and either credentials or explicit public unauthenticated read gate; issue creation/field writes need separate confirmation and live write Go. |
